...
# On failure, shows the last lines of output in an error box.
d_progress_box() {
    local title="$1"; shift
    local cmd_log; cmd_log=$(mktemp)

    # Run command, tee to log and to dialog progressbox
    # progressbox height is DH, width DW
    (
        "$@" 2>&1 | tee "$cmd_log" \
        | dialog --title "$title" \
...
# d_gauge_steps <title> <total_steps> <step_label> <cmd...>
# Runs a command while showing a static gauge at a given percentage.
# Usage: call repeatedly, incrementing step each time.
# Simpler wrapper: d_run_gauge <pct> <title> <label> <cmd...>
d_run_gauge() {
    local pct="$1" title="$2" label="$3"; shift 3
    local cmd_log; cmd_log=$(mktemp)

    # Show gauge while command runs in background
    (
        "$@" >"$cmd_log" 2>&1
        echo $? > "${cmd_log}.rc"
    ) &
    ...
    # Keep gauge displayed until background job finishes
...
# d_multi_gauge <title> <steps_array_name>
# Runs a sequence of labeled steps with a progressing gauge bar.
# steps_array must be: ("label1" "cmd1_arg0" "cmd1_arg1" ... $'\0' "label2" ...)
# Simpler: use run_steps() below.

# run_steps <title> step_label cmd [args]...
# Accepts triplets of (pct label cmd...) terminated by sentinel
# Easier: just call d_progress_box for pacman/AUR (streaming),
# and d_run_gauge for quick ops (format, mount, etc.)
...
d_progress_box "📦 Installation de la base système (pacstrap)" \
    pacstrap -K /mnt \
...
_CHROOT_SCRIPT=$(mktemp /tmp/chroot-XXXXXX.sh)
...
d_progress_box "⚙️  Configuration du système (chroot)" \
    arch-chroot /mnt /bin/bash "$_CHROOT_SCRIPT"
