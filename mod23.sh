...
# _cache_gauge <title> <cache_dir> <total> <cmd...>
# Runs <cmd> in background, polls <cache_dir> for completed .pkg.tar.zst
# files (excluding .part temporaries), and drives dialog --gauge.
# This bypasses all output-buffering issues entirely.
_cache_gauge() {
    local title="$1" cache_dir="$2" total="$3"; shift 3
    local cmd_log; cmd_log=$(mktemp)
    local rc_file; rc_file=$(mktemp)
    local ts_file; ts_file=$(mktemp)   # timestamp reference for newest-package label

    # Baseline: packages already in cache before we start (exclude .sig files)
    local baseline
    baseline=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.sig' ! -name '*.part' \
               2>/dev/null | wc -l)

    # Run the install command in background, capturing all output
    ("$@" >"$cmd_log" 2>&1; echo $? >"$rc_file") &
    local bg_pid=$!

    # Poll loop — feeds dialog --gauge until the background job exits
    (
        local last_label="..."
        while kill -0 "$bg_pid" 2>/dev/null; do
            local current
            current=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.sig' ! -name '*.part' \
                      2>/dev/null | wc -l)
            local done_dl=$(( current - baseline ))
            [[ $done_dl -lt 0 ]] && done_dl=0
            local pct=$(( total > 0 ? done_dl * 100 / total : 0 ))
            [[ $pct -gt 100 ]] && pct=100

            # Label: name of the most recently completed package
            local newest
            newest=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.sig' ! -name '*.part' \
                     -newer "$ts_file" -printf '%T@ %f\n' 2>/dev/null \
                     | sort -n | tail -1 | awk '{print $2}' \
                     | sed 's/-[0-9].*//')
            [[ -n "$newest" ]] && last_label="$newest"

            printf "XXX\n%d\nTéléchargement  %s  (%d/%d)\nXXX\n" \
                "$pct" "$last_label" "$done_dl" "$total"
            sleep 0.4
        done
        echo "100"
    ) | dialog --title "$title" \
...
# Counts expected downloads with pacman -Sp, then installs while watching
# the pacman cache directory to drive dialog --gauge.
pacman_gauge() {
    local title="$1"; shift
    local pkgs=()
    ...
    local total_dl
    total_dl=$(pacman -Sp --print-format '%n' "${pkgs[@]}" 2>/dev/null | wc -l)
    [[ $total_dl -lt 1 ]] && total_dl=20

    _cache_gauge "$title" /var/cache/pacman/pkg "$total_dl" \
        pacman -S --noconfirm --disable-download-timeout "${pkgs[@]}"
}

# pacstrap_gauge <title> <mountpoint> <pkg1> [pkg2 ...]
# Runs pacstrap while polling the cache dir; uses a dynamic count
# since pacman -Sp can't reliably predict total downloads from the live ISO.
pacstrap_gauge() {
    local title="$1" mnt="$2"; shift 2
    local pkgs=()
    for p in "$@"; do [[ -n "$p" ]] && pkgs+=("$p"); done

    local cache_dir="${mnt}/var/cache/pacman/pkg"
    local cmd_log; cmd_log=$(mktemp)
    local rc_file; rc_file=$(mktemp)
    ...
    ("pacstrap" -K "$mnt" "${pkgs[@]}" --disable-download-timeout >"$cmd_log" 2>&1; echo $? >"$rc_file") &
    local bg_pid=$!

    # Mode indéterminé : on affiche le nombre croissant sans plafond connu
    (
        local last_label="..."
        while kill -0 "$bg_pid" 2>/dev/null; do
            local current
            ...
            # Pourcentage basé sur 500 paquets estimés (base+kernel+firmware)
            local pct=$(( done_dl * 100 / 500 ))
            [[ $pct -gt 99 ]] && pct=99  # ne pas atteindre 100% avant la fin réelle
            ...
            printf "XXX\n%d\nTéléchargement  %s  (%d paquets)\nXXX\n" \
                "$pct" "$last_label" "$done_dl"
            sleep 0.4
        done
        echo "100"
    ) | dialog --title "$title" \
               --gauge "Installation en cours..." 8 "$DW" 0 >/dev/tty
...
d_progress_box "⚙️  Configuration de base (chroot)" \
    arch-chroot /mnt /bin/bash "${_CHROOT_CFG#/mnt}"
...
chroot_pacman_gauge() {
    ...
    # Pré-calculer le total depuis l'intérieur du chroot où la DB est correcte
    local total_dl
    total_dl=$(arch-chroot /mnt pacman -Sp --print-format '%n' "${pkgs[@]}" 2>/dev/null | wc -l)
    [[ $total_dl -lt 1 ]] && total_dl=50

    _cache_gauge "$title" /mnt/var/cache/pacman/pkg "$total_dl" \
...
if [[ "$AUR_HELPER" != "none" ]]; then
    chroot_pacman_gauge "AUR Helper — dépendances" go git base-devel
...
d_progress_box "⚙️  Configuration post-install (chroot)" \
    arch-chroot /mnt /bin/bash "${_CHROOT_POST#/mnt}"
