...
if [[ -n "${_AUR_PKGS## }" && "$AUR_HELPER" != "none" ]]; then
    # Écrire dans /mnt/tmp pour que le chroot puisse lire dans /tmp
    mkdir -p /mnt/tmp

    # Écrire un script shell dans le chroot — évite tous les problèmes de quoting
    cat > /mnt/tmp/install-aur.sh << AURSCRIPT
#!/bin/bash
set -e
...
cleanup() {
    rm -f /etc/sudoers.d/aur-install-tmp /tmp/install-aur.sh
}
trap cleanup EXIT

# Installer chaque paquet AUR
su - '${USERNAME}' -c "${AUR_HELPER} -S --noconfirm --needed ${_AUR_PKGS}"
AURSCRIPT

    chmod +x /mnt/tmp/install-aur.sh
    d_progress_box "Paquets AUR" \
        arch-chroot /mnt bash /tmp/install-aur.sh || true
fi
