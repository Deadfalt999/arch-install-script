...
# ══════════════════════════════════════════════════════════
LOG_FILE="/root/ais.md"
MD_LOG="/root/ais.md"
# Filtrer les codes ANSI — dialog écrit directement sur /dev/tty
exec > >(sed 's/\x1b\[[0-9;]*m//g; s/\x1b\[[0-9]*[A-Za-z]//g' | tee -a "$LOG_FILE" >/dev/null) 2>&1

# En-tête Markdown
cat >> "$LOG_FILE" << MDHEADER
# AutoArch — Journal d'installation

| Champ | Valeur |
|---|---|
| Date | $(date '+%Y-%m-%d %H:%M:%S') |
| Script | 1-install-unified.sh |
...
info()    { echo -e "\n${BLUE}[INFO]${NC} $1";    echo "**[INFO]** $1" >> "$LOG_FILE"; }
success() { echo -e "${GREEN}[OK]${NC} $1";       echo "✅ $1" >> "$LOG_FILE"; }
warn()    { echo -e "${YELLOW}[WARN]${NC} $1";    echo "⚠️  $1" >> "$LOG_FILE"; }
error()   { echo -e "${RED}[ERREUR]${NC} $1"; echo "❌ **ERREUR** : $1" >> "$LOG_FILE"; exit 1; }
banner()  {
    echo -e "\n${BOLD}══ $1 ══${NC}"
    echo "" >> "$LOG_FILE"
    echo "## $1" >> "$LOG_FILE"
...
d_progress_box() {
    ...
    cat "$cmd_log" >> "$_LOG"
    cat "$cmd_log" >> "$LOG_FILE" 2>/dev/null || true
    rm -f "$cmd_log"
}
...
_cache_gauge() {
    ...
    # Écrire directement dans LOG_FILE (stdout→/dev/null ne suffit pas)
    cat "$cmd_log" >> "$LOG_FILE" 2>/dev/null || true
    rm -f "$cmd_log"
}
...
pacstrap_gauge() {
    ...
    _cache_gauge "$title" \
        ...
        pacstrap -K "$mnt" --noconfirm "${pkgs[@]}" --disable-download-timeout
}
...
_DE_SEL=$(d_checklist "Environnements de bureau" \
    ...
    \
    "kde"          "KDE Plasma    — Wayland/X11, moderne         [COMPLET]" "on" \
    "gnome"        "GNOME         — Wayland, épuré               [COMPLET]" "off" \
    "cinnamon"     "Cinnamon      — X11, Windows-like (Mint)     [COMPLET]" "off" \
    "mate"         "MATE          — X11, classique, léger         [COMPLET]" "off" \
    "budgie"       "Budgie        — X11/Wayland, élégant (Solus) [COMPLET]" "off" \
    "lxqt"         "LXQt          — X11, très léger (Lubuntu)    [COMPLET]" "off" \
    "xfce"         "XFCE4         — X11, léger et stable          [LÉGER]"  "on" \
    "openbox"      "Openbox       — X11, minimaliste              [LÉGER]"  "off" \
    "i3"           "i3            — X11 tiling (Manjaro i3)       [TILING]" "off" \
    "sway"         "Sway          — Wayland tiling (i3 Wayland)   [TILING]" "off" \
    "hyprland"     "Hyprland      — Wayland tiling (CachyOS)      [TILING]" "off" \
    "bspwm"        "BSPWM         — X11 tiling scriptable         [TILING]" "off" \
    "awesome"      "AwesomeWM     — X11 tiling + Lua              [TILING]" "off")
...
# Normaliser — remplacer newlines/tabs par espaces, supprimer doublons d'espaces
_PKG_SEL=$(echo "$_SEL_NAV $_SEL_MEDIA $_SEL_GAMING $_SEL_EMU $_SEL_BUREAU $_SEL_DEV $_SEL_NET" \
    | tr '\n\t' '  ' | tr -s ' ')

if [[ "$AUR_HELPER" == "none" ]]; then
    for _aur_pkg in brave librewolf waterfox zen-browser vivaldi opera heroic protonup goverlay vkbasalt bottles discord rpcs3 duckstation cemu ryujinx melonds bgb onlyoffice signal obsidian timeshift vscode ananicy veracrypt sudachi lime3ds azahar simple64 flycast redream xenia vita3k shadps4 bizhawk fsuae; do
        _PKG_SEL="${_PKG_SEL//$_aur_pkg/}"
    done
fi

# Normaliser après suppression (peut laisser des espaces multiples)
_PKG_SEL=$(echo "$_PKG_SEL" | tr -s ' ' | sed 's/^ //;s/ $//')
...
d_progress_box "🌐 Miroirs — Sélection en cours..." \
    bash -c "reflector --country France --age 12 --protocol https --sort rate \
        --connection-timeout 5 --download-timeout 15 --latest 20 \
...
# Créer vconsole.conf immédiatement après pacstrap
# pour que mkinitcpio lors des installations suivantes
# ne génère pas de warning "vconsole.conf not found"
mkdir -p /mnt/etc
...
# Re-sync explicite pour s'assurer que multilib est disponible
...
# Sync DB après activation multilib — nécessaire pour pcsx2 et lib32
arch-chroot /mnt pacman -Syy &>/dev/null || true
...
$INSTALL_BUDGIE   && chroot_pacman_gauge "Budgie" \
    budgie-desktop budgie-extras \
    gnome-session gnome-control-center gnome-keyring \
    nautilus network-manager-applet
$INSTALL_OPENBOX  && chroot_pacman_gauge "Openbox" \
    openbox obconf-qt tint2 feh picom \
    lxsession xorg-xinit

# Wayland tiling — nécessite WLR_RENDERER=pixman sur VMware
$INSTALL_SWAY     && chroot_pacman_gauge "Sway" \
    sway swaylock swayidle waybar wofi mako \
    xdg-desktop-portal-wlr grim slurp
$INSTALL_HYPRLAND && chroot_pacman_gauge "Hyprland" \
...
$INSTALL_I3       && chroot_pacman_gauge "i3" \
    i3-wm i3status i3lock dmenu feh picom \
    xorg-xinit
$INSTALL_BSPWM    && chroot_pacman_gauge "BSPWM" \
    bspwm sxhkd rofi picom feh polybar \
    xorg-xinit
...
if [[ "$AUR_HELPER" != "none" ]]; then
    ...
    d_progress_box "AUR Helper — ${AUR_HELPER}" \
        arch-chroot /mnt bash -c "
            ...
            # Cloner
            cd /tmp
            # Cloner avec retry (AUR peut être temporairement indisponible)
            _retry=0
            until git clone https://aur.archlinux.org/${AUR_HELPER}.git; do
                _retry=\$(( _retry + 1 ))
                [[ \$_retry -ge 3 ]] && echo 'AUR inaccessible apres 3 tentatives' && exit 1
...
$PKG_WATERFOX    && _AUR_PKGS="$_AUR_PKGS waterfox-bin"
...
$PKG_MIDORI      && _AUR_PKGS="$_AUR_PKGS midori-bin"
# Multimédia
...
$PKG_GAMEMODE    && _PACMAN_PKGS="$_PACMAN_PKGS gamemode"
$PKG_GAMEMODE    && ! $_IS_VM && _PACMAN_PKGS="$_PACMAN_PKGS lib32-gamemode"
$PKG_MANGOHUD    && _PACMAN_PKGS="$_PACMAN_PKGS mangohud"
$PKG_MANGOHUD    && ! $_IS_VM && _PACMAN_PKGS="$_PACMAN_PKGS lib32-mangohud"
$PKG_PROTONUP    && _AUR_PKGS="$_AUR_PKGS protonup-qt-bin"
$PKG_GOVERLAY    && _PACMAN_PKGS="$_PACMAN_PKGS goverlay"
...
$PKG_PCSX2       && _AUR_PKGS="$_AUR_PKGS pcsx2"
...
$PKG_SNES9X      && _PACMAN_PKGS="$_PACMAN_PKGS snes9x"
$PKG_FCEUX       && _PACMAN_PKGS="$_PACMAN_PKGS fceux"
$PKG_NESTOPIA    && _AUR_PKGS="$_AUR_PKGS nestopia"
$PKG_MUPEN64     && _PACMAN_PKGS="$_PACMAN_PKGS mupen64plus"
$PKG_XEMU        && _AUR_PKGS="$_AUR_PKGS xemu-bin"
...
$PKG_DOSBOX_S    && _AUR_PKGS="$_AUR_PKGS dosbox-staging"
$PKG_VICE        && _PACMAN_PKGS="$_PACMAN_PKGS vice"
$PKG_OPENMSX     && _AUR_PKGS="$_AUR_PKGS openmsx"
# Émulateurs — AUR
...
$PKG_MELONDS     && _AUR_PKGS="$_AUR_PKGS melonds"
$PKG_LIME3DS     && _AUR_PKGS="$_AUR_PKGS lime3ds"
$PKG_AZAHAR      && _AUR_PKGS="$_AUR_PKGS azahar"
$PKG_SIMPLE64    && _AUR_PKGS="$_AUR_PKGS simple64"
$PKG_RPCS3       && _AUR_PKGS="$_AUR_PKGS rpcs3-bin"
$PKG_DUCKSTATION && _AUR_PKGS="$_AUR_PKGS duckstation"
$PKG_SHADPS4     && _AUR_PKGS="$_AUR_PKGS shadps4"
$PKG_VITA3K      && _AUR_PKGS="$_AUR_PKGS vita3k"
$PKG_XENIA       && _AUR_PKGS="$_AUR_PKGS xenia-canary-bin"
$PKG_FLYCAST     && _AUR_PKGS="$_AUR_PKGS flycast-git"
$PKG_REDREAM     && _AUR_PKGS="$_AUR_PKGS redream"
$PKG_DOSBOX_X    && _AUR_PKGS="$_AUR_PKGS dosbox-x"
$PKG_FSUAE       && _AUR_PKGS="$_AUR_PKGS fs-uae"
$PKG_BIZHAWK     && _AUR_PKGS="$_AUR_PKGS bizhawk"
...
$PKG_ONLYOFFICE  && _AUR_PKGS="$_AUR_PKGS onlyoffice-desktopeditors"
...
$PKG_OBSIDIAN    && _PACMAN_PKGS="$_PACMAN_PKGS obsidian"
...
$PKG_VERACRYPT   && _PACMAN_PKGS="$_PACMAN_PKGS veracrypt"
...
if [[ -n "${_AUR_PKGS## }" && "$AUR_HELPER" != "none" ]]; then
    # Écrire dans /mnt/root/ (toujours accessible dans le chroot)
    cat > /mnt/root/install-aur.sh << AURSCRIPT
#!/bin/bash
set -e
...
cleanup() {
    rm -f /etc/sudoers.d/aur-install-tmp /root/install-aur.sh
    # Retirer le swap temporaire si créé
    swapoff /root/aur-build-swap 2>/dev/null || true
    rm -f /root/aur-build-swap
}
trap cleanup EXIT

# Créer un swap temporaire pour éviter OOM pendant les compilations LLVM
RAM_TOTAL=\$(grep MemTotal /proc/meminfo | awk '{print \$2}')
if [[ \$RAM_TOTAL -lt 8388608 ]]; then  # < 8 Go RAM
    echo "RAM < 8 Go détectée — création d'un swap temporaire de 4 Go pour les compilations AUR..."
    dd if=/dev/zero of=/root/aur-build-swap bs=1M count=4096 status=none
    chmod 600 /root/aur-build-swap
    mkswap /root/aur-build-swap -q
    swapon /root/aur-build-swap
    echo "Swap temporaire activé"
fi

# Limiter les jobs de compilation selon la RAM disponible
...
export MAKEFLAGS
echo "MAKEFLAGS=\$MAKEFLAGS (RAM: \${RAM_GB} Go)"

# Configurer makepkg pour utiliser MAKEFLAGS
sed -i "s/^#MAKEFLAGS=.*/MAKEFLAGS=\"\$MAKEFLAGS\"/" /etc/makepkg.conf

# Installer les paquets AUR
su - '${USERNAME}' -c "${AUR_HELPER} -S --noconfirm --needed ${_AUR_PKGS}"
AURSCRIPT

    chmod +x /mnt/root/install-aur.sh
    d_progress_box "Paquets AUR" \
        arch-chroot /mnt bash /root/install-aur.sh || true
fi
...
if [[ "${_IS_VM}" == "true" ]]; then
    ...
    success "Services activés : NetworkManager, vmtoolsd, vmware-vmblock-fuse"
elif [[ "${_OPTIMUS}" == "true" ]]; then
    systemctl enable switcheroo-control
    success "Services activés : NetworkManager, switcheroo-control"
else
    success "Services activés : NetworkManager"
fi

# ── Configuration Wayland (VMware — software rendering) ───
if [[ "${_IS_VM}" == "true" ]]; then
    # Sway, Hyprland et autres WLR-based compositors nécessitent
    # le rendu logiciel sur VMware (pas de GPU Vulkan)
    mkdir -p /etc/environment.d
    cat > /etc/environment.d/wayland-vm.conf << 'EOF'
# VMware — forcer le rendu logiciel pour les compositors Wayland
WLR_RENDERER=pixman
WLR_NO_HARDWARE_CURSORS=1
LIBGL_ALWAYS_SOFTWARE=1
EOF
    success "Variables Wayland software rendering configurées pour VMware"
fi

# ── Configuration BSPWM ──────────────────────────────────
if [[ "${INSTALL_BSPWM}" == "true" ]]; then
    _BSPWM_CFG="/home/${USERNAME}/.config/bspwm"
    _SXHKD_CFG="/home/${USERNAME}/.config/sxhkd"
    mkdir -p "$_BSPWM_CFG" "$_SXHKD_CFG"
    # Config de base bspwmrc
    cat > "$_BSPWM_CFG/bspwmrc" << 'EOF'
#!/bin/sh
sxhkd &
...
EOF
    chmod +x "$_BSPWM_CFG/bspwmrc"
    # Config de base sxhkdrc
    cat > "$_SXHKD_CFG/sxhkdrc" << 'EOF'
# Terminal
super + Return
    xterm
# Launcher
...
super + alt + q
    bspc quit
EOF
    chown -R ${USERNAME}:${USERNAME} "/home/${USERNAME}/.config/bspwm" "/home/${USERNAME}/.config/sxhkd"
    success "BSPWM configuration de base créée"
fi

# ── Configuration Openbox ─────────────────────────────────
if [[ "${INSTALL_OPENBOX}" == "true" ]]; then
    _OB_CFG="/home/${USERNAME}/.config/openbox"
    mkdir -p "$_OB_CFG"
    # Copier la config par défaut
    cp /etc/xdg/openbox/* "$_OB_CFG/" 2>/dev/null || true
    chown -R ${USERNAME}:${USERNAME} "$_OB_CFG"

    # Créer un .desktop de session si manquant
...
EOF
    ...
    success "Openbox configuration créée"
fi

# ── Configuration Budgie ──────────────────────────────────
if [[ "${INSTALL_BUDGIE}" == "true" ]]; then
    # S'assurer que la session Budgie est disponible
...
EOF
    ...
    success "Budgie session desktop créée"
fi
...
| Champ | Valeur |
|---|---|
| Config | ${_CONFIG_LABELS[$CONFIG]:-inconnue} |
| Disque | $DISK |
| Utilisateur | $USERNAME @ $HOSTNAME |
...
# ── Copie dans le système installé ────────────────────────
if mountpoint -q /mnt 2>/dev/null; then
    cp "$LOG_FILE"   /mnt/root/ais.md    2>/dev/null || true
    cp "$LOG_GZ"     /mnt/root/ais.md.gz 2>/dev/null || true
fi
...
\n\
📄 Log : /root/ais.md (compressé : ais.md.gz)" \
    16 66 >/dev/tty

clear >/dev/tty
echo "📄 Log disponible dans /root/ais.md et /root/ais.md.gz" >/dev/tty
