...
_cache_gauge() {
    ...
    (
        ...
        while kill -0 "$bg_pid" 2>/dev/null; do
            ...
            elif [[ $done_dl -eq 0 && $done_inst -gt 0 ]]; then
                # Paquets déjà en cache — installation directe sans téléchargement
                phase="Installation (cache)"
                local pct=$(( done_inst * 100 / ( done_inst > 50 ? done_inst + 10 : 60 ) ))
                [[ $pct -gt 99 ]] && pct=99
