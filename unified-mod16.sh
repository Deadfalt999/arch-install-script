...
# Wraps pacstrap with a live dialog --gauge driven by pacman's (N/N) output.
pacstrap_gauge() {
    local title="$1" mnt="$2"; shift 2
    local pkgs=("$@")
    local cmd_log; cmd_log=$(mktemp)

    # stdbuf -oL forces line-buffered stdout so dialog sees each line immediately.
    # The pipeline: pacstrap → stdbuf tee (log) → awk (emit gauge protocol) → dialog
    # We run the whole pipeline in a subshell and capture its exit via a sentinel file.
    local rc_file; rc_file=$(mktemp)
    (
        ...
        | stdbuf -oL awk '
            /^\(([0-9]+)\/([0-9]+)\)/ {
                # Extract counter and action from lines like:
                #   (3/42) Downloading  linux-firmware-intel-...
                #   (3/42) Installing   linux (6.12-1) ...
                match($0, /^\(([0-9]+)\/([0-9]+)\)[[:space:]]+([A-Za-z]+)[[:space:]]+([^[:space:]]+)/, a)
                n     = a[1]+0
                total = a[2]+0
                action = a[3]
...
# Ensure /mnt/tmp exists (pacstrap creates it, but be explicit)
mkdir -p /mnt/tmp
_CHROOT_SCRIPT=$(mktemp /mnt/tmp/chroot-XXXXXX.sh)
