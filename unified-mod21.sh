...
_cache_gauge() {
    ...
    (
        ...
        while kill -0 "$bg_pid" 2>/dev/null; do
            ...
            if [[ $done_dl -gt 0 && $done_dl -eq $prev_cached && $in_progress -eq 0 && $done_inst -gt 0 ]]; then
                ...
                # total_seen inclut en cours + complétés → donne une estimation du total
                local denom=$(( total_seen > done_dl ? total_seen : done_dl + in_progress + 1 ))
                local pct=$(( done_dl * 49 / denom ))
                [[ $pct -gt 49 ]] && pct=49

                # Nom du dernier paquet en cours de téléchargement (.part)
                local newest_part
                newest_part=$(find "$cache_dir" -maxdepth 1 -name '*.part' \
                              -printf '%T@ %f\n' 2>/dev/null \
                              | sort -n | tail -1 | awk '{print $2}' \
                              | sed 's/-[0-9].*//;s/\.part//')
                # Sinon dernier téléchargé
                ...
                printf "XXX\n%d\n%s  %s  (%d+%d paquets)\nXXX\n" \
                    "$pct" "$phase" "$last_label" "$done_dl" "$in_progress"
            fi
            sleep 0.3
        done
        echo "100"
