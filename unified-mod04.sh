...
# Fallback : lsblk laisse TRAN vide pour de nombreux disques SCSI/VMware.
# /sys/block/<disk>/device/scsi_disk est créé par le sous-système SCSI du
# noyau et est présent sur tout disque SCSI/SAS/virtuel VMware.
if [[ -z "$_TRAN" && -d "/sys/block/${_DISK_NAME}/device/scsi_disk" ]]; then
    _TRAN="scsi"
fi
