...
if [[ -n "${_AUR_PKGS## }" && "$AUR_HELPER" != "none" ]]; then
    ...
    mkdir -p /mnt/tmp
    echo "${_AUR_PKGS}" > /mnt/tmp/aur-packages.txt
    chmod 644 /mnt/tmp/aur-packages.txt
    ...
    d_progress_box "Paquets AUR" \
        arch-chroot /mnt bash -c "
            ...
            # Lire la liste depuis le fichier (évite les problèmes d'expansion shell)
            AUR_LIST=\$(cat /tmp/aur-packages.txt)

            # Installer avec le helper — trap pour nettoyer même en cas d'erreur
            cleanup() { rm -f /etc/sudoers.d/aur-install-tmp /tmp/aur-packages.txt; }
            trap cleanup EXIT

            su - '${USERNAME}' -c '${AUR_HELPER} -S --noconfirm --needed --mflags \"--nocheck\" '\$AUR_LIST" || true
fi
