...
_cache_gauge() {
    ...
    # Baseline: packages already in cache before we start
    local baseline
    baseline=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.part' \
               2>/dev/null | wc -l)
    ...
    (
        ...
        while kill -0 "$bg_pid" 2>/dev/null; do
            local current
            current=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.part' \
                      2>/dev/null | wc -l)
            ...
            local newest
            newest=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.part' \
...
chroot_pacman_gauge() {
    ...
    local pkgs=("$@")
    local total_dl
    total_dl=$(arch-chroot /mnt pacman -Sp "${pkgs[@]}" 2>/dev/null | wc -l)
    [[ $total_dl -lt 1 ]] && total_dl=1
    _cache_gauge "$title" /mnt/var/cache/pacman/pkg "$total_dl" \
...
$INSTALL_KDE      && chroot_pacman_gauge "KDE Plasma" \
    plasma-meta sddm dolphin konsole kate ark gwenview okular spectacle yakuake
