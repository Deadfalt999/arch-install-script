...
_cache_gauge() {
    ...
    # On extrait la commande pacman/pacstrap pour faire une simulation
    ...
    if [[ "$first_cmd" == "pacman" ]]; then
        ...
        # arch-chroot /mnt pacman -S ... → extraire les paquets
        local chroot_pkgs=("${@:3}")  # skip arch-chroot + /mnt
        ...
        # pacstrap -K /mnt pkg1 pkg2... → paquets commencent à l'index 3
        total_expected=$(pacman -Sp --print-format '%n' "${@:3}" 2>/dev/null \
                         | grep -v "^error" | wc -l)
    fi
    [[ $total_expected -lt 1 ]] && total_expected=0  # 0 = inconnu
