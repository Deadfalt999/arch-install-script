...
# Récupère les miroirs HTTPS français depuis l'API Arch
...
for MIRROR in $MIRROR_LIST; do
    ...
    # Test silencieux : télécharge le fichier d'index du miroir (timeout 5s)
    if curl -sf --max-time 5 "${MIRROR%\$repo*}" &>/dev/null; then
        SELECTED_MIRROR="$MIRROR"
        echo -e "\r${GREEN}[OK]${NC} Miroir sélectionné (essai n°${ATTEMPT}) : ${GREEN}${MIRROR}${NC}"
        break
    fi
done
...
# Écrit le miroir retenu dans mirrorlist
