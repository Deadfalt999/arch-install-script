...
pacman_gauge() {
    ...
    pacman -S --noconfirm --disable-download-timeout "${pkgs[@]}" \
        > >(tee "$cmd_log" > "$fifo") 2>&1 &
    local pac_pid=$!

    (
        while IFS= read -r line; do
            # Matches lines like:
            #   (3/42) Downloading  linux-6.12-x86_64.pkg.tar.zst
            #   (3/42) Installing   linux (6.12-1) ...
            if [[ "$line" =~ ^\(([0-9]+)/([0-9]+)\)[[:space:]]+(Downloading|Installing|Upgrading|Checking|Loading)[[:space:]]+([^[:space:]]+) ]]; then
...
# Same as pacman_gauge but wraps pacstrap.
pacstrap_gauge() {
    ...
    (
        ...
        echo "100"
               ...
               --gauge "Démarrage de pacstrap..." 8 "$DW" 0 >/dev/tty
...
# pacman_gauge inside chroot — same fifo-based approach
pacman_gauge() {
    local title="\$1"; shift
    local pkgs=("\$@")
    local cmd_log fifo
