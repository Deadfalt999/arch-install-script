...
# ── Disque cible ──────────────────────────────────────────
# Fonction dédiée : construit et affiche le checklist disques sans sous-shell,
# ce qui préserve le tableau Bash et les descriptions contenant des espaces.
_pick_disk() {
    local _args=() _first="" _dname _dsize _dtran _dmodel _dtype _tag _desc _status
    ...
    while IFS= read -r _dline; do
        ...
        if   [[ "$_dname" == nvme*          ]]; then _dtype="NVMe"
        elif [[ "${_dtran,,}" == "sata"     ]]; then _dtype="SATA"
        elif [[ "${_dtran,,}" == "ide"      ]]; then _dtype="IDE"
        elif [[ "${_dtran,,}" == "usb"      ]]; then _dtype="USB"
        else                                         _dtype="SCSI"
        fi
        ...
        [[ "$_tag" == "$_first" ]] && _status="on" || _status="off"

        _args+=( "$_tag" "$_desc" "$_status" )
    done < <(lsblk -d -n -o NAME,SIZE,TRAN,MODEL,TYPE 2>/dev/null | awk '$5=="disk"' | awk '{$5=""; print $0}')

    [[ ${#_args[@]} -eq 0 ]] && { clear; error "Aucun disque détecté."; }

    dialog --clear --title "Disque cible" \
           --checklist "Sélectionnez le disque cible (⚠️  il sera ENTIÈREMENT EFFACÉ) :" \
           "$DH" "$DW" $(( ${#_args[@]} / 3 )) \
           "${_args[@]}" \
           2>"$_TMP" >/dev/tty
    local rc=$?
    [[ $rc -ne 0 ]] && { clear; error "Installation annulée."; }
}

_pick_disk
# Garder uniquement le premier token (au cas où l'utilisateur en cocherait plusieurs)
DISK=$(cat "$_TMP" | tr -d '"' | awk '{print $1}')
