...
# Pre-counts expected downloads with --print-format, then runs pacstrap
# and tracks only "Downloading" lines to drive dialog --gauge.
...
pacstrap_gauge() {
    ...
    # Count how many packages will actually be downloaded (deps included).
    # --print-format '%n' lists one package name per line.
    local total_dl
    total_dl=$(pacstrap "$mnt" -p --print-format '%n' "${pkgs[@]}" 2>/dev/null | wc -l)
    ...
    (
        ...
        | stdbuf -oL awk -v total="$total_dl" '
            # Count only "Downloading" lines for the progress bar.
            # pacstrap prints: (3/42) Downloading  linux-firmware-20250206-1-any.pkg.tar.zst
            /^\([0-9]+\/[0-9]+\)[[:space:]]+Downloading[[:space:]]/ {
                done++
                # Extract the package filename (strip path/version noise for display)
                match($0, /Downloading[[:space:]]+([^[:space:]]+)/, a)
                pkg = a[1]
                sub(/-[0-9].*/, "", pkg)   # strip version suffix for readability
...
# pacman_gauge inside chroot — stdbuf+awk pipeline, no FIFOs
pacman_gauge() {
    local title="\$1"; shift
    local pkgs=("\$@")
    local cmd_log rc_file
    cmd_log=\$(mktemp)
    rc_file=\$(mktemp)
    local term_cols=\$(tput cols 2>/dev/null || echo 70)

    (
        stdbuf -oL pacman -S --noconfirm --disable-download-timeout "\${pkgs[@]}" \
            2>&1 \
        | stdbuf -oL tee "\$cmd_log" \
        | stdbuf -oL awk -v cols="\$term_cols" '
            /^\(([0-9]+)\/([0-9]+)\)/ {
                match(\$0, /^\(([0-9]+)\/([0-9]+)\)[[:space:]]+([A-Za-z]+)[[:space:]]+([^[:space:]]+)/, a)
                n      = a[1]+0
                total  = a[2]+0
        ...
        ' \
        | dialog --title "\$title" \
                 --gauge "Initialisation..." 8 "\$term_cols" 0 >/dev/tty
        echo "\${PIPESTATUS[0]}" > "\$rc_file"
    )
    local rc=\$(cat "\$rc_file"); rm -f "\$rc_file"
    if [[ "\$rc" -ne 0 ]]; then
        echo -e "\n\${RED}[ERREUR]\${NC} pacman a échoué (code \$rc) — \$title"
        tail -20 "\$cmd_log"
        rm -f "\$cmd_log"
        exit 1
    fi
...
if [[ "\${_IS_VM}" == "true" ]]; then
    ...
    pacman_gauge "Drivers VMware — mesa" \
        mesa xf86-input-vmmouse vulkan-icd-loader
    pacman_gauge "Drivers VMware — open-vm-tools" \
        open-vm-tools xf86-video-vmware 2>/dev/null || \
    pacman_gauge "Drivers VMware — open-vm-tools" \
        open-vm-tools
