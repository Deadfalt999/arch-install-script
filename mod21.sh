...
# Counts expected downloads with pacman -Sp, then runs pacstrap while
# watching the target's cache directory to drive dialog --gauge.
pacstrap_gauge() {
    local title="$1" mnt="$2"; shift 2
    # Filtrer les arguments vides (ex: _UCODE_PKG vide sur VMware)
    local pkgs=()
    for p in "$@"; do [[ -n "$p" ]] && pkgs+=("$p"); done

    local total_dl
    # pacman -Sp liste les URLs à télécharger, en incluant toutes les dépendances
    total_dl=$(pacman -Sp --print-format '%n' "${pkgs[@]}" 2>/dev/null | wc -l)
    [[ $total_dl -lt 1 ]] && total_dl=100  # fallback raisonnable
...
chroot_pacman_gauge() {
    ...
    [[ $total_dl -lt 1 ]] && total_dl=20  # fallback raisonnable
    _cache_gauge "$title" /mnt/var/cache/pacman/pkg "$total_dl" \
