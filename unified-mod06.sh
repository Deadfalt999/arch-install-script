...
# Détermination du type de disque.
# NVMe : nom du device. SATA/IDE/USB : valeur TRAN retournée par lsblk.
# Tout le reste (SCSI, SAS, virtio, vide, inconnu) → SCSI.
if [[ "$_DISK_NAME" == nvme* ]]; then
    _AUTO_DISK="/dev/${_DISK_NAME}"; _DISK_TYPE="NVMe"
elif [[ "${_TRAN,,}" == "sata" ]]; then
    _AUTO_DISK="/dev/sda"; _DISK_TYPE="SATA"
elif [[ "${_TRAN,,}" == "ide" ]]; then
    _AUTO_DISK="/dev/sda"; _DISK_TYPE="IDE"
elif [[ "${_TRAN,,}" == "usb" ]]; then
    _AUTO_DISK="/dev/sda"; _DISK_TYPE="USB"
else
    _AUTO_DISK="/dev/${_DISK_NAME:-sda}"; _DISK_TYPE="SCSI"
fi
