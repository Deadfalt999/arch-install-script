...
# --insecure : affiche '*' à chaque frappe (comportement attendu par l'utilisateur)
d_password() {
    dialog --clear --title "$1" \
...
# ── Disque cible ──────────────────────────────────────────
# Construire la liste des disques pour le checklist dialog.
# Chaque disque = une ligne : tag=/dev/name  desc="SIZE — TYPE [MODEL]"
...
# Compter le nombre de disques sélectionnés
...
# ══════════════════════════════════════════════════════════
#  MIROIRS
#  --sort rate  : classe par taux de synchro publiés (pas de
#                 téléchargement de test → très rapide)
#  --number 10  : garde un pool de 10 miroirs
#  -c / -t      : timeouts courts pour échouer vite si besoin
# ══════════════════════════════════════════════════════════
banner "MIROIRS"
info "Sélection des miroirs (France/Allemagne/Pays-Bas)..."
...
success "Miroir principal : ${SELECTED_MIRROR}"

# ══════════════════════════════════════════════════════════
#  PACSTRAP — BASE SYSTÈME
#  `script -q -c "..." /dev/null` alloue un pseudo-TTY pour
#  que pacman détecte isatty() = true et affiche les barres
#  de progression, même avec le pipe `tee` du logging.
# ══════════════════════════════════════════════════════════
...
# ── Pacman — couleurs et verbosité ────────────────────────
# Active Color + VerbosePkgLists dans pacman.conf pour que
# les barres de progression et les noms de paquets s'affichent
# correctement dans le chroot (stdout peut ne pas être un TTY pur).
export TERM=xterm-256color
sed -i 's/^#Color/Color/'                   /etc/pacman.conf
sed -i 's/^#VerbosePkgLists/VerbosePkgLists/' /etc/pacman.conf
# ILoveCandy : remplace la barre de progression par un petit Pac-Man 🟡
