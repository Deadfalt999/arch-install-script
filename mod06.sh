...
# ══════════════════════════════════════════════════════════
banner "MIROIRS"
info "Récupération de la liste des miroirs français..."

MIRROR_LIST=$(curl -s "https://archlinux.org/mirrorlist/?country=FR&protocol=https&use_mirror_status=on" \
    | grep -E "^#Server" | sed 's/^#Server = //')
...
for MIRROR_TEMPLATE in $MIRROR_LIST; do
    ATTEMPT=$((ATTEMPT + 1))
    echo -ne "\r${YELLOW}Sélection du miroir, essai numéro ${ATTEMPT}...${NC}    "

    # Construit l'URL réelle du fichier core.db (téléchargement réel ~130 Ko)
    TEST_URL="${MIRROR_TEMPLATE/\$repo/core}"
    TEST_URL="${TEST_URL/\$arch/x86_64}/core.db"

    # Télécharge vraiment le fichier — si SSL ou réseau flanche, on passe au suivant
    if curl -sf --max-time 10 --retry 0 -o /dev/null "$TEST_URL" 2>/dev/null; then
        SELECTED_MIRROR="$MIRROR_TEMPLATE"
        echo -e "\r${GREEN}[OK]${NC} Miroir sélectionné (essai n°${ATTEMPT}) : ${GREEN}${MIRROR_TEMPLATE}${NC}"
        break
    fi
done

[[ -z "$SELECTED_MIRROR" ]] && error "Aucun miroir disponible. Vérifie ta connexion internet."

echo "Server = ${SELECTED_MIRROR}" > /etc/pacman.d/mirrorlist
success "mirrorlist mis à jour"
