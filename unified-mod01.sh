...
error() { dialog --title "❌ Erreur fatale" --msgbox "$1" 8 60; clear; echo -e "${RED}[ERREUR]${NC} $1"; exit 1; }
...
d_checklist() {
    local title="$1" label="$2"; shift 2
    dialog --clear --title "$title" \
           --checklist "$label" "$DH" "$DW" $(( $# / 3 )) "$@" 2>"$_TMP"
    local rc=$?; [[ $rc -ne 0 ]] && { clear; error "Installation annulée."; }
...
d_yesno() {
    dialog --clear --title "$1" --yesno "$2" 10 "$DW"
}

# d_msgbox <title> <message>
d_msgbox() {
    dialog --clear --title "$1" --msgbox "$2" 12 "$DW"
}
