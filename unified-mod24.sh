...
d_progress_box "🌐 Miroirs — Sélection en cours..." \
    ...
    echo \"Miroir retenu : \$SELECTED\""

# ── Sync DB & résolution des dépendances ─────────────────
d_progress_box "🔍 Résolution des dépendances..." \
    bash -c "pacman -Sy 2>&1 && echo 'Base de données synchronisée'"
...
# Calculer les totaux une seule fois pour toutes les étapes
_PKGS_BASE=(base base-devel ${KERNELS} linux-firmware ${_UCODE_PKG} \
    networkmanager grub efibootmgr nano vim git curl wget gptfdisk)
_PKGS_BASE=("${_PKGS_BASE[@]}" )  # filtrer vides
_TOTAL_BASE=$(pacman -Sp --print-format '%n' "${_PKGS_BASE[@]}" 2>/dev/null | wc -l)

# Exporter pour _cache_gauge (évite les -Sy répétés)
...
if [[ -n "${_AUR_PKGS## }" && "$AUR_HELPER" != "none" ]]; then
    d_progress_box "Paquets AUR" \
        arch-chroot /mnt bash -c "
            ...
            rm -f /etc/sudoers.d/aur-install-tmp"
fi
