...
# Phase 1 (0→49%): watches cache_dir for new .pkg.tar.zst files
...
_cache_gauge() {
    ...
    (
        local last_label="..." phase="Téléchargement" prev_cached=0

        while kill -0 "$bg_pid" 2>/dev/null; do
            local cached installed
            cached=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.sig' ! -name '*.part' \
                     2>/dev/null | wc -l)
            ...
            if [[ $done_dl -gt 0 && $done_dl -eq $prev_cached && $done_inst -gt 0 ]]; then
                ...
                local newest_dl
                newest_dl=$(find "$cache_dir" -maxdepth 1 -name '*.pkg.tar.*' ! -name '*.sig' ! -name '*.part' \
                            -printf '%T@ %f\n' 2>/dev/null \
                            | sort -n | tail -1 | awk '{print $2}' \
                            | sed 's/-[0-9].*//')
                [[ -n "$newest_dl" ]] && last_label="$newest_dl"

                printf "XXX\n%d\n%s  %s  (%d paquets)\nXXX\n" \
                    "$pct" "$phase" "$last_label" "$done_dl"
            fi
            sleep 0.4
        done
        echo "100"
...
if [[ "$AUR_HELPER" != "none" ]]; then
    ...
    d_progress_box "AUR Helper — ${AUR_HELPER}" \
        arch-chroot /mnt bash -c "
            set -e
            # S'assurer que l'utilisateur existe et a les droits wheel
            id '${USERNAME}' &>/dev/null || useradd -m -G wheel '${USERNAME}'

            # Nettoyer un éventuel clone précédent
            rm -rf /tmp/${AUR_HELPER}

            # Cloner et compiler en tant qu'utilisateur non-root
            cd /tmp
            git clone https://aur.archlinux.org/${AUR_HELPER}.git
            chmod -R 777 /tmp/${AUR_HELPER}
            chown -R '${USERNAME}':'${USERNAME}' /tmp/${AUR_HELPER}

            # makepkg doit tourner en non-root
            # -s = installer les dépendances manquantes
            # -i = installer le paquet résultant
            # --noconfirm = pas d'interaction
            # On utilise su plutôt que sudo (plus fiable dans chroot)
            su - '${USERNAME}' -c 'cd /tmp/${AUR_HELPER} && makepkg -si --noconfirm --needed'

            # Nettoyage
            rm -rf /tmp/${AUR_HELPER}
            echo '${AUR_HELPER} installé avec succès'"
fi
